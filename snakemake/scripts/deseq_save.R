...
ids <- suppressMessages(bitr(rownames(dds), fromType=ids_in, toType=c("ENTREZID", "ENSEMBL", "SYMBOL", "GENENAME"), OrgDb=sp_arr["orgdb"]))

result_array = lapply(result_array, function(x) {
  x = rownames_to_column(data.frame(x), ids_in)
...
all = lapply(names(result_array), function (x) {
  write.table(result_array[[x]], file=paste0(outdir, x, "_all.txt"), row.names = F, quote = F, sep = "\t" )
})

# export significant hits
filtered = lapply(names(result_array), function (x) {
  write.table(result_array[[x]] %>% filter(padj < 0.05), file=paste0(outdir, x, "_diffexp.txt"), row.names = F, quote = F, sep = "\t" )
})
...
sample_expression = data.frame(counts(dds, normalized=T), row.names = rownames(counts(dds)))
